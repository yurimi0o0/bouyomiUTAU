...
def vowel_of(token: str | None) -> str | None:
    if not token or token in {"~", "_", "__"}:
        return None
    for vowel, chars in VOWELS.items():
        if token[-1] in chars:
            return vowel
    return None


def _fit_vowel(source: array, target: int, rate: int) -> array:
    """Fit a sustained vowel without resampling it, so its recorded pitch is retained."""
    if target <= 0 or not source:
        return array("h")
    if target <= len(source):
        return source[:target]
    loop_start = min(len(source) - 1, len(source) // 3)
    minimum = min(len(source) - 1, loop_start + max(1, round(rate * 0.04)))
    maximum = min(len(source), loop_start + max(2, round(rate * 0.08)))
    if minimum < maximum:
        start_slope = source[loop_start] - source[max(0, loop_start - 1)]
        loop_end = min(range(minimum, maximum), key=lambda i: abs(source[i] - source[loop_start]) + abs((source[i] - source[i - 1]) - start_slope))
    else:
        loop_end = len(source)
    loop = source[loop_start:loop_end] or source[-1:]
    result = array("h", source)
    while len(result) < target:
        result.extend(loop[:target - len(result)])
    return result[:target]


def _render_mora(clip: array, entry: OtoEntry, rate: int, target: int, continuation: bool = False) -> array:
    if not clip:
        return clip
    consonant = min(len(clip) - 1, round(rate * min(entry.consonant_ms or 65, target * 1000 / rate * 0.7) / 1000))
    fixed = 0 if continuation else min(len(clip), target, consonant)
    onset = clip[:fixed]
    vowel_source = clip[consonant:] or clip[-1:]
    vowel_target = max(0, target - len(onset))
    rendered = onset + _fit_vowel(vowel_source, vowel_target, rate)
...
def synthesize(voicebank: Path, text: str, speed: float = 1.0, crossfade_ms: int = 25, rate: int = 44100, mora_ms: int = 170, naturalness: float = 0.8) -> tuple[bytes, list[str]]:
    ...
    for index, token in enumerate(tokens):
        ...
        if clip:
            ...
            continuation = token in VOWELS and vowel_of(previous) == token
            if continuation:
                factor *= 0.72
            target = max(1, round(rate * mora_ms * factor / 1000 / max(speed, 0.25)))
            clip = _render_mora(clip, entry, rate, target, continuation)
        configured_fade = round(rate * (entry.overlap_ms or min(crossfade_ms, max(8, entry.preutter_ms * 0.35))) / 1000)
        if clip and token in VOWELS and vowel_of(previous) == token:
            configured_fade = max(configured_fade, round(rate * 0.035))
