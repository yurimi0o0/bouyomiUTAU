...
internal static class MonophoneSynthesizer
{
    ...
    {
        ['あ'] = "あかがさざただなはばぱまゃやらわぁ", ['い'] = "いきぎしじちにひびぴみりゐぃ",
        ['う'] = "うくぐすずつぬふぶぷむゅゆるぅ", ['え'] = "えけげせぜてでねへべぺめれゑぇ",
        ['お'] = "おこごそぞとどのほぼぽもょよろをぉ"
    };
    ...
    public static void Synthesize(string voicebankPath, string text, string outputPath, double speed, int crossfadeMs, int moraDurationMs, double naturalness)
    {
        ...
        for (var tokenIndex = 0; tokenIndex < tokens.Count; tokenIndex++)
        {
            ...
            var factor = MoraFactor(token, previous, following, naturalness);
            var continuation = token.Length == 1 && Vowels.ContainsKey(token[0]) && VowelOf(previous) == token[0];
            if (continuation) factor *= 0.72;
            var targetMs = moraDurationMs * factor / Math.Max(speed, 0.25);
            var clip = RenderMora(source.Samples, source.Rate, entry, targetMs, continuation);
            ...
            if (continuation) configuredOverlap = Math.Max(configuredOverlap, (int)Math.Round(OutputRate * 0.035));
    ...
    static char? VowelOf(string? token)
    {
        if (string.IsNullOrEmpty(token) || token is "~" or "_" or "__") return null;
        var last = token[^1];
        foreach (var pair in Vowels) if (pair.Value.Contains(last)) return pair.Key;
        return null;
    }

    static short[] FitVowel(short[] source, int targetLength)
    {
        if (targetLength <= 0 || source.Length == 0) return [];
        if (targetLength <= source.Length) return source[..targetLength];
        var loopStart = Math.Min(source.Length - 1, source.Length / 3);
        var minimum = Math.Min(source.Length - 1, loopStart + Math.Max(1, (int)Math.Round(OutputRate * 0.04)));
        var maximum = Math.Min(source.Length, loopStart + Math.Max(2, (int)Math.Round(OutputRate * 0.08)));
        var startSlope = source[loopStart] - source[Math.Max(0, loopStart - 1)];
        var loopEnd = minimum < maximum
            ? Enumerable.Range(minimum, maximum - minimum).MinBy(i => Math.Abs(source[i] - source[loopStart]) + Math.Abs((source[i] - source[i - 1]) - startSlope))
            : source.Length;
        var loopLength = Math.Max(1, loopEnd - loopStart);
        var result = new short[targetLength];
        Array.Copy(source, result, source.Length);
        for (var i = source.Length; i < targetLength; i++) result[i] = source[loopStart + (i - source.Length) % loopLength];
        return result;
    }

    static short[] ResampleRate(short[] source, int sourceRate)
    {
        if (sourceRate == OutputRate) return source;
        var length = Math.Max(1, (int)Math.Round((double)source.Length * OutputRate / sourceRate));
        return Enumerable.Range(0, length).Select(i => source[Math.Min(source.Length - 1, (int)((long)i * sourceRate / OutputRate))]).ToArray();
    }

    static short[] RenderMora(short[] source, int sourceRate, OtoEntry entry, double targetMs, bool continuation)
    {
        ...
        var targetFixed = continuation ? 0 : Math.Min(targetLength, Math.Max(1, (int)Math.Round(fixedMs * OutputRate / 1000)));
        var result = new short[targetLength];
        ...
        var vowel = ResampleRate(source[vowelStart..]);
        var fitted = FitVowel(vowel, targetLength - targetFixed);
        Array.Copy(fitted, 0, result, targetFixed, fitted.Length);
